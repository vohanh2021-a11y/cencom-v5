...
//   W6-reg (dot gop registry) — 16 fn moi, cach suy theo META×MATRIX nhu tren:
//     ke_toan (10 = ketoan 8 + ledger 2): tinhGiaVon, reconcileKho, vatInvoiceSave,
//       phieuChiCreate, congNoList, ledgerReport, kyClose, kyOpen, ledgerPost,
//       ledgerList → [ke_toan, xem/tao/ky]. MATRIX chi vai ketoan mang module
//       ke_toan (du 6 nhan xem/tao/ky/vat/chi/baocao → chu dispatch→loi checkLock
//       thong; admin bypass) → ketoan=true, giamdoc/xuong/kho=false (403 dispatch,
//       dung v3.6: so cai = specialized duty).
//     khach_hang (4): khachHangList/Get → [khach_hang, xem], khachHangSave/Del →
//       [khach_hang, tao] — MATRIX ketoan:['xem','tao'] → ketoan=true, vai khac false.
//     baoduong (2): baoDuongList → [xe, xem] → ca 4 role deu co xe:['xem'] → ALLOW
//       het; baoDuongTao → [xe, tao] → KHONG vai non-admin nao mang xe.tao (tien le
//       xeCreate) → DENY het, admin-only.
